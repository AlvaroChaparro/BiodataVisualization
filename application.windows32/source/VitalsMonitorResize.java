...
public class VitalsMonitorResize extends PApplet {
...
public void setup(){
  ...
  try {
    ...
    while ((line = br.readLine()) != null) {
      ...
      ecgRecords.add(PApplet.parseFloat(values[index]));
      // Valor de control si cambia de segundo
      current = PApplet.parseInt(PApplet.parseFloat(values[0]));
      // Si cambia el valor del tiempo al siguiente segundo
      if (current != control){
        if ((line = br2.readLine()) != null){
          values = line.split(",");
          ecgNumerics.add(PApplet.parseInt(values[index2]));
          control = current;
        }
      }else{
  ...
  try {
    ...
    while ((line = br.readLine()) != null) {
      String[] values = line.split(",");
      spo2Records.add(PApplet.parseFloat(values[index]));

      current = PApplet.parseInt(PApplet.parseFloat(values[0]));
      if (current != control){
        if ((line = br2.readLine()) != null){
          values = line.split(",");
          spo2Numerics.add(PApplet.parseInt(values[index2]));
          control = current;
        }
      }else{
  ...
  try {
    ...
    while ((line = br.readLine()) != null) {
      String[] values = line.split(",");
      rpmRecords.add(PApplet.parseFloat(values[index]));
      ...
      if (current != control){
        if ((line = br2.readLine()) != null){
          values = line.split(",");
          rpmNumerics.add(PApplet.parseInt(values[index2]));
          control = current;
        }
      }else{
...
public void drawSensorData(int file, float[] dataArray){
  ...
  for(int i=1; i<width*13/16; i++){
    ...
    switch(file){
      case 1:
        ...
        point0 = PApplet.parseInt(height - (scaling*height/3*0.99f) - height*2/3*0.99f);
        scaling = (ecgArray[i]-minECG)/(maxECG-minECG);
        point1 = PApplet.parseInt(height - (scaling*height/3*0.99f) - height*2/3*0.99f);
        break;
      case 2:
        scaling = (spo2Array[i-1]-minSPO2)/(maxSPO2-minSPO2);
        point0 = PApplet.parseInt(height - (scaling*height/3*0.99f) - height/3*0.99f);
        scaling = (spo2Array[i]-minSPO2)/(maxSPO2-minSPO2);
        point1 = PApplet.parseInt(height - (scaling*height/3*0.99f) - height/3*0.99f);
        break;
      case 3:
        scaling = (rpmArray[i-1]-minRPM)/(maxRPM-minRPM);
        point0 = PApplet.parseInt(height - (scaling*height/3*0.99f) - 5);
        scaling = (rpmArray[i]-minRPM)/(maxRPM-minRPM);
        point1 = PApplet.parseInt(height - (scaling*height/3*0.99f) - 5);
        break;
    }
...
public void drawECGNumbers(){
  ...
  textFont(fontBold, ((width*3/16)+(height*4/18))*0.28f-(100/height));
  print("\n");
  text(str(HR_num), width*14.5f/16,height*4/18,width*3/16,height*2/9);
  
  textFont(fontNarrow, ((width*3/16)+(height*1/18))*0.11428f);
  text(" ECG",width*14/16,height*1/18,width*1/16,height*1/9);
  
  textFont(fontBold, ((width*3/16)+(height*1/18))*0.08571f);
  text("BPM ",width*15/16,height*1/18,width*1/16,height*1/9);
}

  // SPO2 Numbers
public void drawSPO2Numbers(){
  ...
  textFont(fontBold, ((width*3/16)+(height*4/18))*0.28f);
  text(str(SPO2_num), width*14.5f/16,height*10/18,width*3/16,height*2/9);
...
public void drawRPMNumbers(){
  ...
  textFont(fontBold, ((width*3/16)+(height*4/18))*0.28f);
  text(str(RPM_num), width*14.5f/16,height*16/18,width*3/16,height*2/9);
...
public void setpalette() {
  ...
  palette = new int[strings.length];
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "VitalsMonitorResize" };
    if (passedArgs != null) {
